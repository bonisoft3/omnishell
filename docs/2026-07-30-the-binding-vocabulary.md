...
| `data-exit-motion="none"` | a list whose rows leave at once, with no exit motion: the departing row is removed in the pass that loses it. For a list whose filter selects one row, where a leaving row and its replacement side by side are a layout jump rather than a transition. `none` is the only value, and a slot has no rows to leave, so either mistake is refused |
